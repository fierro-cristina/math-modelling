...
def TD_chooseOptimalMove(state, player, epsilon):
    ...
    optim_move_idx = np.argmax(curr_state_values)

    if np.random.uniform(0,1) <= epsilon:       # Exploration
        optim_move = random.choice(empty_cells)
        print('Agent decides to explore! Action taken = ' + str(optim_move))
        epsilon *= 0.99
    else:   #Exploitation
        optim_move = moves[optim_move_idx]
        print('Agent decides to exploit! Action taken = ' + str(optim_move))
    return optim_move
...
for iteration in range(num_iterations):
    ...
    while current_state == "Not Done":
        ...
        if current_player_idx == 0:     # AI_X's turn
            print("\nAI X's turn!")
            block_choice = TD_chooseOptimalMove(game_state, players[current_player_idx], epsilon)
        ...
        if winner is not None:
            ...
            if str(winner)=='X':
                X_wins_array.append(winner)
            else:
                O_wins_array.append(winner)
        else:
            current_player_idx = (current_player_idx + 1)%2
        if current_state is "Draw":
            print("Draw!")
            draws_array.append(current_state)
        # time.sleep(1)

total_X_wins = len(X_wins_array)
total_O_wins = len(O_wins_array)
total_draws = len(draws_array)

objects = ('X', 'O', 'Draws')
pos = np.arange(len(objects))
overall = [total_X_wins, total_O_wins, total_draws]

print('\nTraining Complete!\n')
print('-'*20)
print('\nStats for ' + str(num_iterations) + ' iterations: \n')
print("Total X wins: ", total_X_wins)
print("Total O wins: ", total_O_wins)
print("Total draws: ", total_draws)

# plt.bar(pos, overall, align = 'center', alpha = 0.5)
# plt.xticks(pos, objects)
# plt.ylabel('number of wins')
# plt.title('Statistics per ' + str(num_iterations) + ' iterations.')
# plt.show()
